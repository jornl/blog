...
class TestingServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        AssertableInertia::macro('doesntHaveResource', function (string $key, JsonResource $resource) {
            $this->has($key);
            expect($this->prop($key))->not->toContain($resource->response()->getData(true));
        ...
        TestResponse::macro('assertMissingResource', function (string $key, ResourceCollection|JsonResource $resource) {
            return $this->assertInertia(fn (AssertableInertia $inertia) => $inertia->doesntHaveResource($key, $resource));
        });
